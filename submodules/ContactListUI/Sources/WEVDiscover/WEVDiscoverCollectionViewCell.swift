...
class WEVDiscoverCollectionViewCell: UICollectionViewCell {
    ...
    public var tiktokModel: TikTokVideo? = nil {
        didSet {
            updateTiktokView()
        }
    }
    ...
    private func updateYoutubeView() {
        ...
        liveLabel.isHidden = !(model.isLive ?? true)
    ...
    private func updateTiktokView() {
        guard let model = tiktokModel else { return }
        channelImageView.image = WEVChannel.tiktok.smallImage
        channelNameLabel.text = model.title ?? ""
        liveLabel.isHidden = false
        var numberStr = "\(model.viewerCount)"
        var unit = "viewers"
        ...
        amountLabel.text = "  \(numberStr) \(unit)  "
        //print(model.thumbnailUrl.removingPercentEncoding ?? "")
        if let url = model.thumbnailUrl.removingPercentEncoding {
