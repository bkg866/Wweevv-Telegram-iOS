            ...
            if let channelId = subscribes[index].youTubeChannelId {
                Alamofire.request("https://www.googleapis.com/youtube/v3/activities", method: .get, parameters: ["part":"snippet,id,contentDetails","channelId": channelId,"key": LJConfig.Youtube.apiKey]).responseJSON { response in
                    ...
                    do {
                        ...
                        for var item in subscribeParsedData.items {
                            item.subscribeId = subscribes[index].id
                            arrSubVideo.append(item)
                        }
                        totalVideos -= 1
                        if totalVideos == 0 {
...
extension WEVSubscribeControllerNode: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView,
        ...
        let archive = UIContextualAction(style: .normal,
            ...
            let alert = UIAlertController.init(title: "Warning!", message: "do you want to unsubscribe channel of this video?", preferredStyle: .alert)
            alert.addAction(.init(title: "Cancel", style: .default, handler: nil))
            alert.addAction(.init(title: "Unsubscribe", style: .destructive, handler: { okaction in
                if let id = self?.arrSubsribedVideo[indexPath.row].subscribeId {
                   self?.doPerformRemoveWatchLater(id: id)
                }
               if let channelId = self?.arrSubsribedVideo[indexPath.row].snippet.channelID {
                   self?.arrSubsribedVideo.removeAll(where: {$0.snippet.channelID == channelId})
                   DispatchQueue.main.async {
