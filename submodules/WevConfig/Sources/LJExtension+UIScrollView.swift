...
extension LJExtension where Base: UIScrollView {
    ...
    /// 添加头部刷新
    public func addMJReshreHeader(delegate: LJScrollViewRefreshDelegate) {
        let scrollView = base
    ...
    /// 添加底部刷新
    public func addMJReshreFooter(delegate: LJScrollViewRefreshDelegate) {
        let scrollView = base
    ...
    /// 停止刷新动画
    public func endRefreshing(isHeader: Bool) {
        if isHeader {
...
/// 配合MJRefresh使用
public protocol LJScrollViewRefreshDelegate: NSObject {
