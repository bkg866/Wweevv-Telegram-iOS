    ...
    @objc private func shareButtonAction() {
        ...
        guard let code = applyCodeTextField.text, code.count >= 5 else {
            DispatchQueue.main.async {
                ...
                self.showTootlipPopUp(message: "Please enter valid referral code")
            }
            return
        }
           ...
           .decoded(to: [WevUser].self)
            ...
            if user.userId == self.context.account.peerId.id._internalGetInt64Value() {
                DispatchQueue.main.async {
                    ...
                    self.showTootlipPopUp(message: "You can't use your referral code your self")
                    self.shareButton.isEnabled = true
                }
                return
            }
            ...
            guard point.isEmpty else {
                DispatchQueue.main.async {
                    ...
                    self.showTootlipPopUp(message: "You have already used redeem points")
                    self.shareButton.isEnabled = true
                }
                return
            }
           ...
           .decoded(to: [PointsType].self)
            ...
            guard let type1 = pointType.first(where: {$0.type == 1}), let type2 = pointType.first(where: {$0.type == 2}) else {
                DispatchQueue.main.async {
                    ...
                    self.showTootlipPopUp(message: "Point type data are missing")
                    self.shareButton.isEnabled = true
                }
                return
            }
            ...
            guard let pointIds = insertedPoints else {
                DispatchQueue.main.async {
                    ...
                    self.showTootlipPopUp(message: "something went wrong")
                    self.shareButton.isEnabled = true
                }
                return
            }
            ...
            DispatchQueue.main.async {
                ...
                self.showTootlipPopUp(message: "You have successfully applied referral code.")
                self.shareButton.isEnabled = true
