...
extension WEVWatchLaterControllerNode: UITableViewDelegate, UITableViewDataSource {
    ...
    /*func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            self.arrWatchLater.remove(at: indexPath.row)
    ...
    func tableView(_ tableView: UITableView,
        ...
        let archive = UIContextualAction(style: .normal,
            ...
            let alert = UIAlertController.init(title: "Warning!", message: "do you want to remove from watch later?", preferredStyle: .alert)
            alert.addAction(.init(title: "Cancel", style: .default, handler: nil))
            alert.addAction(.init(title: "Remove", style: .destructive, handler: { okaction in
                if let id = self?.arrWatchLater[indexPath.row].id {
